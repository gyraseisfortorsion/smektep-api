...
class AssignmentSubmissionService(ServiceBase[AssignmentSubmission, AssignmentSubmissionCreate, AssignmentSubmissionResubmit]):
    ...
    async def transcribe(self, submission_id: str, db: Session):
        ...
        for _ in range(2):
            msg = chain.invoke(
            [
                AIMessage(content="You are a useful bot that is especially good at transcribing student submitted works, no matter if handwritten or printed. You should always reply in the format of markdown. Image may contain tables and diagrams, if tables or diagrams are present represent them accordginly with markdown"),
                HumanMessage(
                    content=[
                        {"type": "text", "text": "Transcribe image contents. Reply in the markdown format. Take into consideration that most of the text and math symbols will be in Russian! Don't leave anything out, return all extracted texts with no comments. Image may contain tables and diagrams, if tables or diagrams are present use markdown to represent them."},
                        {
                            "type": "image_url",
                            "image_url": {
    ...
    async def transcribe_gpt(self, submission_id: str, db: Session):
        ...
        for _ in range(3):
            ...
            [
                ...
                HumanMessage(
                    content=[
                        {"type": "text", "text": "Extract all texts from image. Take into consideration that most of the text and math symbols will be in Russian! Don't leave anything out, return all extracted texts with no comments. Don't reply in LaTeX, always respond in HTML"},
                        {
                            "type": "image_url",
                            "image_url": {
                                "url": f"data:image/jpeg;base64,{submission_image_base64}",
